...
DT_TENANT_APPS, DT_TENANT_BASE = build_dt_urls(dt_env_id=DT_ENVIRONMENT_ID, dt_env_type=DT_ENVIRONMENT_TYPE)
...
do_file_replace(pattern="runtimeChange.sh", find_string="DT_ENDPOINT_PLACEHOLDER", replace_string=DT_TENANT_BASE)
...
run_command(["kubectl", "create", "secret", "generic" ,"dt-details", f"--from-literal=DT_ENDPOINT={DT_TENANT_BASE}", f"--from-literal=DT_API_TOKEN={DT_API_TOKEN}"])
